...
def test_weather_agent_fills_plan():
    ...
    assert "2025-08-01" in result.weather.forecast_by_day
...
def test_weather_forecast_content():
    ...
    # Day 1: code 0 = Clear sky, 26-33°C
    assert "Clear sky" in result.weather.forecast_by_day["2025-08-01"]
    assert "33" in result.weather.forecast_by_day["2025-08-01"]

    # Day 2: code 61 = Light rain → should trigger rain packing tip
...
def test_weather_agent_handles_geocode_failure():
    ...
    # safe_run should catch the error, not crash
